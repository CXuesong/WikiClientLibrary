...
public class WikiLink
{
    ...
    private static async Task<WikiLink?> ParseInternalAsync(WikiSite? site, IWikiFamily? family, string text, int defaultNamespaceId,
        bool exceptionOnFailure)
    {
        ...
        if (title.Contains('\ufffd'))
        {
            if (exceptionOnFailure)
                throw new ArgumentException(string.Format(Prompts.ExceptionTitleIllegalCharacter1, "\uFFFD 'REPLACEMENT CHARACTER'"),
                    nameof(text));
            return null;
        }
