...
public class CargoPreEvaluationTranslator : ExpressionVisitor
{
    ...
    protected override Expression VisitMethodCall(MethodCallExpression node)
    {
        if (node.Object == null && node.Method.DeclaringType == typeof(CargoFunctions))
        {
            ...
            {
                nameof(CargoFunctions.Like) => new CargoBinaryOperationExpression(" LIKE ", Visit(node.Arguments[0]),
                    Visit(node.Arguments[1]), node.Type),
                ...
                nameof(CargoFunctions.HoldsLike) => new CargoBinaryOperationExpression(" HOLDS LIKE ", Visit(node.Arguments[0]),
